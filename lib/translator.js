  ...
  toggle() {
     ...
     if(editor && editor.getLastSelection() && editor.getLastSelection().getText()){
        ...
        function translate(contexts) {
            ...
            if (!paragraph) {
                atom.notifications.addWarning('该行为空，中断翻译!');
                return selection.insertText(contexts.slice(i, contexts.length).join("\n\n"));
            }
            ...
            return that.translationProvider.translate(that.translationProvider.getDefaultTraslationLang(), paragraph, function (word) {
                ...
                ++i;
                content = content + translatedParagraph;
                selection.insertText(content);
                if (i === contexts.length - 1) return atom.notifications.addSuccess("翻译完成!");
                translate(contexts);
            })
        }
        translate(contexts);
